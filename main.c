...
//  CGP
//
//  Created by Shravanthi Madhugiri on 19/06/21.
//
...
void options(int id){  //menu options
...
void DrawCircle(float cx, float cy, float cz, float r, int num_segments)
{
    ...
    int ii;
    float theta, x, y, i , j;
    if(clouds == 1) j=num_segments/2;  //half circle
    else j=num_segments;
    glBegin(GL_POLYGON);
        for(ii = 0; ii < j; ii++)
        {
...
void drawText(char text[], int x, int y, int z)
{
    ...
    // color
    glColor3f(1, 1, 1);
    // Position of text to be printed
    glRasterPos3f(x, y, z);
...
void drawMainScene()
{
    ...
    char h2[] = "ROAD ACCIDENT";
    char h3[] = "by";
    char name1[] = "Shravanthi Madhugiri";
    char usn1[] = "1BG18CS108";

    char message[] = "Press 1 to start and to move to the next scene.";

    drawText(h1, -110, 200, 0);
    drawText(h2, -60, 165, 0);
    drawText(h3, 10, 135, 0);
    drawText(name1, -70, 90, 0);
    drawText(usn1, -40, 70, 0);
    drawText(message, -170, -200, 0);
...
void StoryScene(){
    ...
    glBegin(GL_QUADS);
        ...
        glVertex2f(-670.0,-350.0);
    glEnd();
    char *story[] = {"India tops the world in road crash deaths and injuries.",
        "It has 1 per cent of the world's vehicles but accounts for 11 per cent of all road crash deaths.",
        "It witnesses 53 road crashes every hour, killing 1 person every 4 minutes",
        "In 2018, there were around 151 thousand deaths due to road accidents in India.",
        "It is important to always wear your seatbelts when you are inside the vehicle.",
        "Please ensure your safety by following all necessary precautions."
                    };
     printLines(story, 0, 6,-400, 200);
     drawCandles();
...
int main(int argc, char** argv) {
  ...
  glutCreateWindow("Road Accident");
  ...
  glutAddMenuEntry("Light candles",1);
